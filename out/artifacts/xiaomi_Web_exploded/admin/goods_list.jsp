...
<body>
    ...
    <div class="rightinfo">
	    ...
	    <table class="tablelist">
			...
			<c:forEach items="${pt.result}" var="goods" varStatus="i">
				<tr>
					<td><input name="chk" type="checkbox" value="${goods.gid}" /></td>
					...
					<td><img src="<%=imgPath%>${goods.pic}" width="80px"/></td>
					<td>
					...
					<td>${goods.version}</td>
					<td><fmt:formatDate value="${goods.product_date}" pattern="yyyy-MM-dd HH:mm:ss"></fmt:formatDate></td>
					<td>
						<a href="<%=basePath%>goods?method=findGoodsById&gid=${goods.gid}">修改</a>
					</td>
				</tr>
			</c:forEach>
	        </tbody>
	    ...
	    <div class="pagin">
	        ...
	        <ul class="paginList">
		         <li class="paginItem"><a href="<%=basePath%>goods?method=findAllGoods&currentPage=1">首页</a></li>
		         <li class="paginItem"><a href="<%=basePath%>goods?method=findAllGoods&currentPage=${pt.prePage}">上一页</a></li>
		         <li class="paginItem"><a href="<%=basePath%>goods?method=findAllGoods&currentPage=${pt.nextPage}">下一页</a></li>
		         <li class="paginItem"><a href="<%=basePath%>goods?method=findAllGoods&currentPage=${pt.totalPages}">尾页</a></li>
	        </ul>
	    </div>
	    <div class="tip">
...
<script>
	function add_goods()
	{
		location.href="<%=basePath%>goods?method=findAllCategory";
	}
function batchDelete()
{
	...
	if(confirm("确认删除吗？"))
	{
		location.href="<%=basePath%>goods?method=deleteGoodsById&ids="+ids;
	}
}
</script>
</body>
