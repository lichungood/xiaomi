...
<body>
    ...
    <div class="formtitle"><span>类别信息</span></div>
		...
		<form action="<%=basePath%>cate" method="post">
	    ...
	    <ul class="forminfo">
		    ...
		    <li><label>是否启用</label>
			    <cite>
					<input name="state" type="radio" value="1" <c:if test="${category.state==1}">checked="checked"</c:if>/>是

				    <input name="state" type="radio" value="0" <c:if test="${category.state==0}">checked="checked"</c:if>/> 否
			    </cite>
		    </li>
