...
<%--${trolleys}--%>
    ...
    <div class="trolley_background">
        ...
        <div class="tro_close_bot ">
            ...
            <p class="tro_close_p "> <a href="<%=basePath%>index?method=indexInfo">继续购物 </a>  | 共<span>${trolleys.size()}</span>种商品</p>
...
<script>
  ...
  function  addOrDeleteNumber(tid,number)
  {
      ...
      {
          number=1;
          alert("至少保留一件，移除购物车请点击删除");
      }
      else
      {
          location.href="<%=basePath%>trolley?method=updateTrolleyById&tid="+tid+"&number="+number;
      }
  }
  //购物车的删除
    function delTrolley(tid)
    {
        if(confirm("确认删除吗？"))
        {
            location.href="<%=basePath%>trolley?method=delTrolleyById&tid="+tid;
        }
    }
   function pay(sumPrice,goodsCount)
   {
       if(goodsCount==0||sumPrice==0)
       {
           location.href="<%=basePath%>index?method=indexInfo";
       }
       else
       {
           location.href="<%=basePath%>orders?method=insertOrders&sumPrice="+sumPrice+"&goodsCount="+goodsCount;
       }
   }
</script>
</body>
